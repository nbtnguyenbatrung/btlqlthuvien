...
namespace librarian.view
{
    public partial class Frm_type : Form
    {
        ...
        private void bindingNavigatorthem_Click(object sender, EventArgs e)
        {
            ...
            DataRow m_Row = m_typectrl.ThemDongMoi();
            m_typectrl.ThemType(m_Row);
        ...
        private void bindingNavigatorxoa_Click(object sender, EventArgs e)
        {
            ...
            else if ((MessageBox.Show("Bạn có chắc chắn muốn xóa ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                ...
                foreach (DataGridViewRow row in dgvuser.SelectedRows)
                {
                    ...
                    {
                        conn.Open();
                        string select1 = "Select bookId from tb_book where typeId=" + row.Cells["typeId"].Value;
                        ...
                        OleDbCommand cmd = new OleDbCommand("delete from tb_type where typeId =" + row.Cells["typeId"].Value, conn);
                        cmd.ExecuteNonQuery();
        ...
        {
            if (KiemTraTruocKhiLuu("nameType") == true)
            {
        ...
        void TimKiemUser()
        {
            ...
            m_typectrl.TimTheoTen(txttimkiem.Text);
        ...
        {
            m_typectrl.HienThi(dgvuser, bindingNavigatoruser);
        }
